...
- [x] Operational platform — UI-01 theme persistence, DEV-03 API keys (SHA-256 hash, ApiKeyAuthenticationHandler with scheme forwarding), AUTH-05 UsersController, AUTH-09 AuditLog (GET /audit + emission from auth/users/apikeys/alerts/schedules), MTR-04/05 AlertRule + AlertIncident + AlertEvaluatorService (1-min cadence, webhook notifier, auto-resolve), PSM-07 ScheduledExecution + NCrontab worker; 72 backend + 37 frontend + 10 Pester tests, all five admin pages + router entries wired; [docs/MOCK_DATA.md](docs/MOCK_DATA.md) documents the demo-vs-real data strategy — `Demo:SeedEnabled` flag is the only switch.
- [x] MVP polish — UI-06 ErrorBoundary at root, PSM-02 ScriptDetailPage, AUTH-06+ADM-01 Profile + self-service password change, TBL-04 pagination UI on Executions, ADM-02 Health page; 51 frontend tests (+14).
- [x] E2E — Playwright config + 10 specs across auth/dashboard/scripts; CI `e2e` job spins up `docker-compose.dev.yml`, waits for the stack, runs the suite, uploads the report. `continue-on-error: true` so flakes don't block merges.
